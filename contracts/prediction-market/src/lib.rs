...
impl PredictionMarket {
    ...
    pub fn claim_winnings(env: Env, bettor: Address, market_id: u64) -> i128 {
        ...
        // Check if bettor won
        let won = match (pos.outcome, m.resolved_outcome) {
            (Outcome::A, Outcome::A) | (Outcome::B, Outcome::B) => true,
            _ => false,
        };

        if !won {
            return 0; // No winnings for losing bets
        }
    ...
    pub fn place_bet_reputation_weighted(
        ...
        let weighted_amount = amount.saturating_mul(weight_multiplier as i128);

        // Calculate tokens received using AMM formula with weighted amount
        let (tokens_a, tokens_b) = match outcome {
            Outcome::A => {
                let tokens_out =
                    calculate_tokens_out(m.outcome_a_reserve, m.outcome_b_reserve, weighted_amount);
                (tokens_out, 0)
            }
            Outcome::B => {
                let tokens_out =
                    calculate_tokens_out(m.outcome_b_reserve, m.outcome_a_reserve, weighted_amount);
                (0, tokens_out)
            }
            _ => panic!("invalid outcome"),
        };
...
fn get_agent_reputation(env: &Env, agent: &Address) -> i128 {
